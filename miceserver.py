...
async def create_lobby(request: LobbyCreateRequest):
    ...
    lobbies[username]["connections"] = {}
    lobbies[username]["last_heartbeats"] = {username: time.time()}
    lobbies[username]["heartbeat_monitor_task"] = None
    clients[lobby_id] = []
...
async def join_lobby(request: LobbyJoinRequest):
    ...
    lobby["connections"][username] = None
    lobby["last_heartbeats"][username] = time.time()
...
async def start_game(request: StartGameRequest):
    ...
    await start_heartbeat_monitor(lobby_id)
        ...
        while True:
            try:
                ...
                if action == "create":
                    ...
                    lobbies[username]["connections"] = {username: websocket}
                    lobbies[username]["last_heartbeats"] = {username: time.time()}
                    lobbies[username]["heartbeat_monitor_task"] = None
                    ...
                    lobby["positions"][username] = {"x": 0.0, "y": 0.0, "z": 0.0}
                    lobby["rotations"][username] = {"x": 0.0, "y": 0.0, "z": 0.0, "w": 1.0}
                    lobby["connections"][username] = websocket
                    lobby["last_heartbeats"][username] = time.time()
                    ...
                    if username == lobby["creator"]:
                        ...
                        if lobby.get("heartbeat_monitor_task") is not None:
                            lobby["heartbeat_monitor_task"].cancel()
                            lobby["heartbeat_monitor_task"] = None
                        ...
                        if username in lobby["players"]:
                            ...
                            if "connections" in lobby and username in lobby["connections"]:
                                del lobby["connections"][username]
                            if "last_heartbeats" in lobby and username in lobby["last_heartbeats"]:
                                del lobby["last_heartbeats"][username]
                            await notify_clients(lobby_id, {
                                "lobby_id": lobby_id,
                                "players": lobby["players"],
                                "status": lobby["status"]
                    ...
                    for c, l in lobbies.items():
                        if l["lobby_id"] == lobby_id:
                            lobby = l
                            break
                    if lobby and username in lobby.get("last_heartbeats", {}):
                        lobby["last_heartbeats"][username] = time.time()
...
async def finish_server_timer(lobby_id: str):
    ...
    print(f"Timer finished for lobby {lobby_id}")
async def start_heartbeat_monitor(lobby_id: str):
    lobby = None
    for c, l in lobbies.items():
        if l["lobby_id"] == lobby_id:
            lobby = l
    ...
    if not lobby:
        return
    if lobby["heartbeat_monitor_task"] is not None:
        lobby["heartbeat_monitor_task"].cancel()
    lobby["heartbeat_monitor_task"] = asyncio.create_task(heartbeat_monitor_task(lobby_id))
async def heartbeat_monitor_task(lobby_id: str):
    lobby = None
    for c, l in lobbies.items():
        if l["lobby_id"] == lobby_id:
            lobby = l
    ...
    try:
        while True:
            ...
            if lobby.get("status") != "started":
                break
            now = time.time()
            disconnected = [u for u, t in list(lobby.get("last_heartbeats", {}).items()) if now - t > 3.0]
            for u in disconnected:
                await handle_player_disconnect(lobby_id, u)
    except asyncio.CancelledError:
        pass
    except Exception as e:
        print(f"Error in heartbeat monitor for lobby {lobby_id}: {e}")
    finally:
        lobby["heartbeat_monitor_task"] = None
async def handle_player_disconnect(lobby_id: str, username: str):
    lobby = None
    creator = None
    for c, l in lobbies.items():
        if l["lobby_id"] == lobby_id:
            ...
            creator = c
            break
    if not lobby or username not in lobby.get("players", []):
        return
    if username == lobby["creator"]:
        if lobby_id in clients:
            for client in list(clients[lobby_id]):
                try:
                    ...
                    pass
        if lobby.get("timer_task") is not None:
            lobby["timer_task"].cancel()
            lobby["timer_task"] = None
        if lobby.get("heartbeat_monitor_task") is not None:
            lobby["heartbeat_monitor_task"].cancel()
        ...
        if lobby_id in clients:
            del clients[lobby_id]
        del lobbies[creator]
        print(f"Lobby {lobby_id} deleted because creator {username} disconnected")
    else:
        if username in lobby["players"]:
            lobby["players"].remove(username)
        ...
        if username in lobby["positions"]:
            del lobby["positions"][username]
        if username in lobby.get("rotations", {}):
            del lobby["rotations"][username]
        if "ready_players" in lobby and username in lobby["ready_players"]:
            lobby["ready_players"].remove(username)
        if "connections" in lobby and username in lobby["connections"]:
            if lobby_id in clients and lobby["connections"][username] in clients[lobby_id]:
                clients[lobby_id].remove(lobby["connections"][username])
            del lobby["connections"][username]
        if "last_heartbeats" in lobby and username in lobby["last_heartbeats"]:
            del lobby["last_heartbeats"][username]
        await notify_clients(lobby_id, {
        ...
        print(f"Player {username} disconnected due to inactivity in {lobby_id}")
