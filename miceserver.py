...
last_ping: Dict[str, Dict[str, float]] = {}
ping_check_tasks: Dict[str, asyncio.Task] = {}
...
async def start_ping_checker(lobby_id: str):
    try:
        while True:
            ...
            if lobby_id in last_ping:
                for username, last_ping_time in last_ping[lobby_id].items():
                    if current_time - last_ping_time > 5:
                        if username in lobby["players"]:
                            players_to_remove.append(username)
            
            for username in players_to_remove:
                if username in last_ping.get(lobby_id, {}):
                    del last_ping[lobby_id][username]
                    
                if username in lobby["players"]:
                    if username == lobby["creator"]:
                        print(f"Creator {username} disconnected from lobby {lobby_id}, closing lobby")
                        await close_lobby_creator_disconnect(lobby_id)
                        return
                    else:
                        print(f"Player {username} disconnected from lobby {lobby_id}, removing from game")
                        await remove_disconnected_player(lobby_id, username)
                        
    except asyncio.CancelledError:
        print(f"Ping checker task cancelled for lobby {lobby_id}")
        raise
    except Exception as e:
        print(f"Error in ping checker for lobby {lobby_id}: {e}")

async def close_lobby_creator_disconnect(lobby_id: str):
    lobby = None
    creator_key = None
    for c, l in lobbies.items():
        if l["lobby_id"] == lobby_id:
    ...
    if lobby_id in ping_check_tasks:
        ping_check_tasks[lobby_id].cancel()
        del ping_check_tasks[lobby_id]
        
    if lobby_id in last_ping:
        del last_ping[lobby_id]
        
    if lobby_id in clients:
        for client in clients[lobby_id]:
            try:
                await client.send_json({
                    "error": "Lobby closed by creator disconnect",
    ...
    if creator_key and creator_key in lobbies:
...
async def remove_disconnected_player(lobby_id: str, username: str):
    lobby = None
    for c, l in lobbies.items():
        if l["lobby_id"] == lobby_id:
            lobby = l
    ...
    print(f"Removed disconnected player {username} from lobby {lobby_id}")
...
async def create_lobby(request: LobbyCreateRequest):
    ...
    clients[lobby_id] = []
    last_ping[lobby_id] = {username: time.time()}
...
async def join_lobby(request: LobbyJoinRequest):
    ...
    if lobby["lobby_id"] not in last_ping:
        last_ping[lobby["lobby_id"]] = {}
    last_ping[lobby["lobby_id"]][username] = time.time()
...
async def start_game(request: StartGameRequest):
    ...
    if lobby_id not in ping_check_tasks:
        ping_check_tasks[lobby_id] = asyncio.create_task(start_ping_checker(lobby_id))
                ...
                if action == "create":
                    ...
                    if username == lobby["creator"]:
                        ...
                        if username in lobby["players"]:
                            ...
                            if lobby_id in last_ping and username in last_ping[lobby_id]:
                                del last_ping[lobby_id][username]
                            await notify_clients(lobby_id, {
                                "lobby_id": lobby_id,
                                "players": lobby["players"],
                    ...
                    if lobby_id and lobby_id in last_ping:
                        last_ping[lobby_id][username] = time.time()
                       
                    await websocket.send_json({"action": "pong"})
                    print(f"Ping received from {username} in lobby {lobby_id}, sent pong")
